...
const AppliedJobOpp = () => {
  ...
  const [displayedJobCount, setDisplayedJobCount] = useState(3);
  ...
  useEffect(() => {
    ...
    const GetSeeker = async () => {
      ...
      const appliedJobsId = appliedJobs.map((currJob) => currJob.jobId);

      const filteredJobs = allJobs.filter((currJob) =>
        appliedJobsId.includes(currJob._id)
      );
  ...
  return (
    <div className="ml-10 mt-3 my-card-div">
      ...
      {referredJob.length===0?<div className="d-flex justify-content-center relative">
            <img
              src={NotFoundImg}
              alt="not found"
              className="w-[220px] h-[220px] mt-8"
            />
            <div className="absolute top-0 left-42 border-2 border-blue-300 mascot-text rounded-lg">
              <span className="font-poppins font-semibold text-[18px] bg-gradient-to-r from-green-400 to-blue-500 bg-clip-text text-transparent">
                You haven't applied anywhere yet. <a href="/joblist" className="no-underline text-blue cursor-pointer">Start Applying!!</a>
              </span>
            </div>
          </div>:
        <div className="row">
