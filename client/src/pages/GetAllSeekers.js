...
function GetAllSeekers() {
    ...
    const GetAll = async () => {
        axios.get("http://localhost:8000/api/v1/seekers/").then(response=>setSeeker(response.data))
    ...
    return (
        <div>
                ...
                <MDBTableBody>
                    {seekers.map((seeker) => (
                        <tr key={seeker._id}>
                            ...
                            <td>
                                <button type="button" className="btn btn-danger" onClick={(e)=>handleDelete(e,seeker._id)}>
                                    Delete
                                </button>
            ...
            <Link to="/admin">
                <button className="btn btn-success btn-lg mb-1" style={{ marginLeft: '10px' }}>Back to Dashboard</button>
            </Link>
        </div>
    );
}
